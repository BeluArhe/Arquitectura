...
\begin{table}[h]
\caption{Instrucciones de la Máquina IAS}
\begin{tabular}{|c|c|c|l|}
\hline
Opcode & Opcode Hex & Símbolo & RTL \\
\hline
...
00000111 & 0x07 & ADD IM(X)I & $[AC]\leftarrow[AC] + \text{IM}(X)I$ \\
00000110 & 0x06 & SUB M(X) & $[AC]\leftarrow[AC] - [X]$ \\
00001000 & 0x08 & SUB IM(X)I & $[AC]\leftarrow[AC] - \text{IM}(X)I$ \\
00001011 & 0x0B & MUL M(X) & $[AC]\leftarrow[AC] \times [X];\, [MQ]\leftarrow\text{bits más significativos}$ \\
00001100 & 0x0C & DIV M(X) & Dividir AC por M(X); colocar el cociente en MQ y el resto en AC \\
00010100 & 0x14 & LSH & $[AC]\leftarrow[AC] \times 2;\, \text{desplazar una posición a la izquierda}$ \\
00010101 & 0x15 & RSH & $[AC]\leftarrow[AC] \div 2;\, \text{desplazar una posición a la derecha}$ \\
\hline
\end{tabular}
\end{table}
